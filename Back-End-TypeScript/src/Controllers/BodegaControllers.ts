import { addItemToBodegaSer, deleteBodegaSer, getBodegaSer, getBodegasSer, insertBodegaSer, updateBodegaSer } from '../Services/BodegaService'
...
export const addItemToBodega = async ({ params, body }: Request, res: Response) => {
  try {
    const { sucursal } = params;
    const { itemId } = body;

    const responseItem = await addItemToBodegaSer(sucursal, itemId);
    if (!responseItem) {
      return res.status(404).json({ error: 'Bodega no encontrada', sucursal });
    }
    res.status(200).json({
      message: 'Item agregado correctamente a la bodega',
      item: responseItem,
    });
